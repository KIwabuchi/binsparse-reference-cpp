...
void bench_hdf5(const std::filesystem::path& file_path,
                const std::filesystem::path& binsparse_file) {
...
void bench_metall(const std::filesystem::path& file_path,
  ...
  auto* manager = new metall::manager(metall::create_only, binsparse_file);
...
int main(int argc, char** argv) {
  ...
  std::filesystem::path binsparse_file = argv[2];
  std::string mode = argv[3];

  if (mode == "h5") {
    H5::DSetCreatPropList prop;
    prop.setDeflate(0);
    bench_hdf5(file_path, binsparse_file);
  } else if (mode == "metall") {
    bench_metall(file_path, binsparse_file);
  } else {
